...
module DB
  ...
  describe 'Import' do
    ...
    it "One row" do
      expect{ ImportClient.import Import.csv_table('client','spec/import_data') }.to \
    ...
    context 'entities' do
      it 'adds one entity when second entity blank' do
        expect{ ImportClient.import Import.csv_table 'client_one_entity','spec/import_data' }.to \
          change(Entity, :count).by 1
      end

      it 'ordered by creation' do
        ImportClient.import Import.csv_table('client','spec/import_data')
