require_relative '../../../lib/stage/patch_property'

describe PatchAddress2, :stage do
  ...
  it 'returns unmatched data unmolested' do
    ...
    patch = PatchAddress2.new patch: [row(human_ref: 20, value: 8)]
    expect(patch.cleanse originals: input).to eq [row(human_ref: 10, value: 4)]
  end

  it 'returns patch data when id match' do
    input = [row(human_ref: 10, value: 4)]
    patch = PatchAddress2.new patch: [row(human_ref: 10, value: 8)]
