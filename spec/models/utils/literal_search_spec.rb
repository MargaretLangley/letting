...
describe LiteralSearch, type: :model do
  ...
  describe '#go' do
    it 'return nil when no match' do
      expect(LiteralSearch.search(type: 'Property', query: '100').go)
        .to eq nil
    end
    ...
    it 'return an exact client ref' do
      ...
      expect(LiteralSearch.search(type: 'Client', query: '101').go)
        .to eq client
    end

    it 'return an exact user' do
      user = user_create nickname: 'george'
      expect(LiteralSearch.search(type: 'User', query: 'george').go)
        .to eq user
    end

    it 'return an exact charge cycle' do
      cycle = charge_cycle_create name: 'Mar/Sep'
      expect(LiteralSearch.search(type: 'ChargeCycle', query: 'Mar/Sep').go)
        .to eq cycle
    end
  end
  ...
  describe 'ordering' do
    it 'searches for the queried type before any other type' do
      ...
      expect(LiteralSearch.search(type: 'Client', query: '100').go)
        .to eq client
    end
