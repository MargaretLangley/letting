...
RSpec.describe ReRunMaker, type: :model do
  ...
  describe 'invoice_date' do
    it 'defaults to today' do
      ...
      expect(rerun.invoices.first.invoice_date).to eq Time.zone.today
    end

    it 'can set date' do
      invoices = [invoice_new(invoice_date: '2000/01/01')]
      rerun = ReRunMaker.new invoices: invoices, invoice_date: '2010/02/02'
      expect(rerun.invoices.first.invoice_date).to eq Date.new(2010, 2, 2)
    end
  end
  ...
  describe 'comments' do
    it 'sets comments' do
      ...
      expect(rerun.invoices.first.comments.size).to eq 1
      expect(rerun.invoices.first.comments.first.clarify).to eq 'a comment'
    end
    ...
    it 'if comments unset it leaves them blank' do
      ...
      expect(rerun.invoices.first.comments.size).to eq 0
    end
  end
end
